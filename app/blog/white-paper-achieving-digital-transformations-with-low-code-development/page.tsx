...
export const metadata = {
  title: 'White Paper: Achieving Digital Transformations with Low Code Development | Appdraft Blog',
  description: 'This paper is intended to set out how organisations can (and are) leveraging Low Code development on Platform as a Service (\'PaaS\') infrastructure to help ex...',
};

export default function BlogPost() {
  const post = posts.find((p) => p.slug === 'white-paper-achieving-digital-transformations-with-low-code-development')!;
  const relatedPosts = posts.filter((p) => p.category === post.category && p.slug !== post.slug).slice(0, 3);

  const content = `<!-- wp:paragraph -->
<p>

How 'low code' development and Platform as a Service infrastructure is helping businesses deliver connected, agile software applications
